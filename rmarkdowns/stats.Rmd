...
```{r, warning=FALSE, message=FALSE}
goal_8_7_linear <- ggplot(sdr_data, aes(x = goal_8_score, 
...
- Edit the code chunk above to plot a linear regression of 2 different sdg scores 
  ...
  - in 4-6 sentences, come up with a hypotheses for the positive or negative correlation between the 2 goals
   ...
   HYPOTHESIS: The plot above shows that there is a positive correlation between SDG 7 and SDG 8. With SDG 7 being affordable and clean energy and SDG 8 being decent work and economic growth, the positive correlation could be attributed to the fact that affordable and clean energy allows better productivity and industrial support for companies. For example, improved clean energy systems for corporate infrastructure and transportation allows systems among multiple communities to be more sustainable. Another reason could be that a growing industry for clean energy creates more jobs for citizens. 
      
      - are there any points that don't follow the trend (outliers)
- Yes, most outliers lie within Africa.
        
        - which country(s) are outliers?
- Some outliers include Iceland, Benin, Guinea, Senegal, Albania, and Gabon.
