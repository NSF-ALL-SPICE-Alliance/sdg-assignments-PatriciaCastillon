...
HYPOTHESIS: Countries in Europe and Africa lean farther away from each other, resulting in very opposite clusters. This may be due to the fact that more SDG's have been accomplished in European countries.
...
```{r}
importance_df <- as.data.frame(rf_sanita$importance)
```
...
ggplot(importance_df_top_10, aes(x = `%IncMSE`, y = reorder(variable, `%IncMSE`))) +
  ...
  theme_minimal() +
  labs(title = "Most Important Variables in Ensuring Access to Water and Sanitation",
       subtitle = "Top 10",
       y = "SDG Indicator",
...
```{r}
pdp::partial(rf_sanita, pred.var = "normalized_score_sdg6_water", plot = TRUE)
```
...
```{r}
pd <- pdp::partial(rf_sanita, pred.var = c("normalized_score_sdg6_water", "normalized_score_sdg7_elecac"))
# Default PDP
plotPartial(pd)
```
